...
public interface UserCircleRepository extends CrudRepository<UserCircle, Integer>{
	...
	* */
	@Query("select u.circleName from UserCircle u where u.username = :username")
	List<String> findCircleNameByUserName(@Param("username") String username);

	@Query("select u from UserCircle u where u.username = :username")
	UserCircle findByUserName(@Param("username") String username);	
